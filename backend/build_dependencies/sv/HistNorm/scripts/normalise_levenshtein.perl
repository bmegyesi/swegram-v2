...
# If the word is in the stoplist, choose the normalised version present in the stoplist
		...
		if($candidate=~/^\|/){ # several candidates - choose the most frequent one
		    $bestcandidate=selectBestCandidate($candidate);
		}
		print "$bestcandidate\n";
...
# Else if the word has been normalised before, choose the same normalisation as previously
		...
		print "$cache{$lcwordform}\n";
		print STDERR "cache: $lcwordform = $cache{$lcwordform}\n";
	    }
...
#	    elsif(($check==1 && $wordform=~/^[A-ZÅÄÖÉÈÁÀ]/)){
#		print "$wordform\n";
#		print STDERR "do not normalise: $wordform\n";
#	    }
...
# contains punctuations or digits
		...
		print STDERR "Select best candidate for wordform: $lcwordform ($bestcandidates)\n";
		my $bestcandidate="";
		if($bestcandidates eq ""){ 
		    $bestcandidate=$wordform;
		    $cache{$lcwordform} = $bestcandidate;
		}
		    ...
		    $bestcandidate=selectBestCandidate($bestcandidates);
		    $cache{$lcwordform} = $bestcandidate;
		}
		    print "$bestcandidate\n";
