...
class SharedAttributeMixin:
    ...
    text_uuid = Column(String(length=255))
        ...
        return {
            **{
                c.name: getattr(self, c.name) for c in self.__table__.columns if c.name in set({
                    "text_uuid", "language", "filename", "labels", "activated",
        ...
        for p_index, paragraph in enumerate(paragraphs, 1):
            ...
            for s_index, sentence in enumerate(sentences, 1):
                ...
                for t_index, token in enumerate(tokens, 1):
                    ...
                    # del token["text_id"]
                    token_instance = Token(text_id=f"{p_index}.{s_index}", token_id=t_index, sentence_id=sentence_instance.id, **token)
