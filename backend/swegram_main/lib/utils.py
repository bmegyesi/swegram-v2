...
# FCT = TypeVar("FCT", bound=Iterator[List[Union[Dict, str, str], str]])
...
T = TypeVar("T", bound=Tuple[List[List[List[str]]], Dict[str, str]])
TS = TypeVar("TS", bound=List[Tuple[List[List[List[str]]], Dict[str, str]]])
...
def read_conll_text(input_path: Path) -> Any:
    """read conllu text, it returns a text and possible meta as labels"""
    file_content = FileContent(input_path).get()
    return _read_conll_text(file_content=file_content)
...
def read_conll_file(input_path: Path) -> TS:
    """Read conllu text"""
    texts: TS = []
    file_content = FileContent(input_path).get()
