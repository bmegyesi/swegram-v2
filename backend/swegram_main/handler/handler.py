...
@StatisticLoading
def load_sentence(text_uuid: str, lines: ST, language: str, parsed: bool = True) -> Sentence:  # pylint: disable=unused-argument
    """Load sentence from conll text"""
    return Sentence(text_uuid=text_uuid, language=language, tokens=[load_token(line, language) for line in lines])


@StatisticLoading
def load_paragraph(text_uuid: str, lines: PT, language: str, parsed: bool = True) -> Paragraph:
    """Load paragraph from conll text"""
    return Paragraph(text_uuid=text_uuid, language=language, sentences=[load_sentence(text_uuid, s, language, parsed=parsed) for s in lines])
...
def load_text(text: TT, labels: Dict[str, str], language: str, filename: Path, parsed: bool = True) -> Text:
    ...
    text_uuid: str = get_content_md5("".join([t for p in text for s in p for t in s]).encode())
    paragraphs: List[Paragraph] = [load_paragraph(text_uuid=text_uuid, lines=p, language=language, parsed=parsed) for p in text]
    return Text(paragraphs=paragraphs, text_uuid=text_uuid, language=language, filename=filename, labels=labels)
