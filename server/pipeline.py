...
class Pipeline:
    ...
    @TaskDecorator(task_name="tokenize")
    ...
    @TaskDecorator(task_name="normalize")
...
if __name__ == "__main__":
    p = Pipeline(language="sv", filename="test.txt")
    p.run("tokenize")
    p.run("normalize")
    p.run("tag")
    p.run("parse")