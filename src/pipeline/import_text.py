...
from scripts.helpers import rm_blanks, str_to_bool
from scripts.process import has_metadata_initial_final, metadata_content_checker, remove_feff
from statistics.general_func import Count_features
from statistics.lexical_func import Lexical_features
from statistics.morph_func import Morph_features
from statistics.readability_func import Readability_features
from statistics.syntactic_func import Syntactic_features, is_a_ud_tree, dep_paths
...
class Sentence:
    text_id = None
    tokens = []
    # types is a list of counters [(type1, num1),...,(type-n, num-n)]
    types = []
    """features"""
    general = None
    readability = None
    # if ud_tree is false, we skip the syntactic feature extraction.
    ud_tree = True
    morph = None
    lexical = None
    syntactic = None
...
def create_token(lang, t, parsed, compound_originals=False):
    ...
    if compound_originals:
        ...
        if not parsed:
            ...
            token.upos = t[5]
            token.xpos = t[6]
            # ??? the output of file: ufeats is placed before feats
            token.feats = t[7]
            if lang == 'en':
                token.head = t[8]
                token.deprel = t[9]
                ...
                # the difference between English and Swedish
                token.ufeats = t[8]
                token.head = t[9]
                token.deprel = t[10]
                token.deps = t[11]
...
def create_paragraphs(text, lang, parsed):
    ...
    if text.raw_contents_list:
        ...
        # to add an end marker to include the last paragraph for looping
        text_lines = rm_blanks(text.raw_contents_list) + ['$END_MARKER$']
    else:
    ...
    for x in range(len(content)):
        ...
        # '\n' is a breaker between sentences when it appears in the beginning of the sentence
            ...
            if sentence.tokens:
                ...
                if parsed:
                    if not is_a_ud_tree(sentence):
                        # we ask the user to take care of the text if the parsing is not correct
                        sentence.ud_tree = False
                        sentences.append(sentence)
                        sentence = Sentence()
                        sentence.tokens = []
...
def import_annotated_text(request, lang, path, label):
    checkNormalization = str_to_bool(
        request.POST.get('checkNormalization', 'false'))
    checkPOS = str_to_bool(request.POST.get('checkPOS', 'false'))

    has_not_error, error_msgs = uploaded_file_checker(
        path, lang, True, checkNormalization, checkPOS)
    if has_not_error:
        text = create_text(lang, path, label, True,
                           checkNormalization, checkPOS, annotated=True)
        return True, text
    else:
...
def get_sentence_stats(lang, sentence, parsed):
    ...
    sentence.readability = vars(
        Readability_features(sentence, lang, level='sent'))

    if parsed:
        # use vars() to serialize the object to json data
        sentence.morph = vars(Morph_features(sentence, lang, level='sent'))
...
def get_paragraph_stats(lang, p, parsed):
    ...
    # Function here to be implemented
    p.readability = vars(Readability_features(p, lang, level='para'))
    if parsed:
        p.morph = vars(Morph_features(p, lang, level='para'))
        p.morph_average = vars(Morph_features(
            p, lang, algorithm='average', level='para'))

        p.lexical = vars(Lexical_features(p, lang=lang, level='para'))
        p.lexical_average = vars(Lexical_features(
            p, lang=lang, algorithm='average', level='para'))

        p.syntactic = vars(Syntactic_features(p, level='para'))
        p.syntactic_average = vars(Syntactic_features(
            p, algorithm='average', level='para'))
    return p
...
def get_text_stats(lang, text, parsed):
    ...
    if parsed:
        ...
        text.morph_average = vars(Morph_features(
            text, lang, algorithm="average", level='text'))

        text.lexical = vars(Lexical_features(text, lang=lang, level='text'))
        text.lexical_average = vars(Lexical_features(
            text, lang=lang, algorithm="average", level="text"))

        text.syntactic = vars(Syntactic_features(text, level='text'))
        text.syntactic_average = vars(Syntactic_features(
            text, algorithm="average", level="text"))
...
def import_text(request, lang, label, path, eligible, check_if_normalizd=False):
    ...
    text = create_text(lang, path, label, eligible,
                       checkNormalization, checkPOS)
...
class Labels:
    ...
    # labels structure {tag1:value1, tag2:value2, ..., tagn:valuen}
    def _add(self, text_id, labels):
...
def create_text(lang, filename, label, eligible, checkNormalization, checkPOS, annotated=False):
    ...
    if rm_blanks(text.raw_contents_list):
        ...
        if annotated:
            ...
            #             print('line', line)
            text.paragraphs = create_annotated_paragraphs(
                [line for line in text.raw_contents_list if line.strip()], lang, checkPOS)
        else:
            text.paragraphs = create_paragraphs(text, lang, checkPOS)
        text.sentences = [
            sent for p in text.paragraphs for sent in p.sentences]
...
def uploaded_file_checker(path, lang, annotated, normalized, parsed, line_index=1):
    ...
    with open(path, mode='r', encoding='utf-8-sig') as f:
        # to check if the annotated sentence has a standard ud tree.
        sentence = []
        line = f.readline()
        sentence_index = None
        while line:
            ...
            if has_label:
                has_error, error_dict = metadata_content_checker(
                    metadata_content)
                if has_error:
                    has_not_error = False
            ...
            if parsed is True and sentence_index and sentence and sentence_index != tabs[0].strip().lstrip():
                try:
                    check_ud_tree = is_a_ud_tree(
                        sentence, upload_annotated=True)
            ...
            if lang == 'en':
                check_xpos = pos_checker(
                    xpos, PT_TAGS, 'xpos', tabs, line_index)
            elif lang == 'sv':
                check_xpos = pos_checker(
                    xpos, SUC_TAGS, 'xpos', tabs, line_index)
            if check_xpos is not True:
                has_not_error = False
                error_msgs.append(check_xpos)

            if lang == 'sv':  # to follow the order when checking
                check_ufeats = ufeats_checker(
                    tabs[-6], UFEATS, tabs, line_index)
