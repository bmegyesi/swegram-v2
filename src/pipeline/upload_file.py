...
def labels2line(label_dict):
    return ''.join([
        ...
        config.METADATA_DELIMITER_LEBAL.join([config.METADATA_DELIMITER_TAG.join(
            [key, value]) for key, value in label_dict.items()]),
...
def write_text2db(t):  # input text instance
    ...
    text = Text.objects.create(
        ...
        syntactic_average=json.dumps(
            t.syntactic_average) if t.parsed else None,
    ...
    for paragraph in t.paragraphs:
        p = Paragraph.objects.create(
            ...
            morph_average=json.dumps(
                paragraph.morph_average) if t.parsed else None,
            lexical=json.dumps(paragraph.lexical) if t.parsed else None,
            lexical_average=json.dumps(
                paragraph.lexical_average) if t.parsed else None,
            syntactic=json.dumps(paragraph.syntactic) if t.parsed else None,
            syntactic_average=json.dumps(
                paragraph.syntactic_average) if t.parsed else None,
...
def upload_annotated_file(request, lang, *args, **kwargs):
    ...
    if os.path.splitext(filename)[1] == '.csv':
        ...
        with open(os.path.join(upload_location, textfile), 'w') as new_text_file:
            with open(os.path.join(upload_location, filename), newline='') as csvfile:
                text_reader = csv.reader(
                    csvfile, delimiter=config.COLUMN_DELIMITER)
        ...
        with open(os.path.join(upload_location, textfile), 'w', encoding='utf-8-sig') as new_text_file:
            ...
            for index, sheet_name in enumerate(sheet_names):
                ...
                if index % 2 == 0:
                    ...
                    if metadata == 'NaN':
                        new_text_file.write(
                            '%s%s' % (config.METADATA_INITIAL, config.METADATA_FINAL))
                    else:
                        new_text_file.write('%s%s%s' % (config.METADATA_INITIAL, config.METADATA_DELIMITER_LEBAL.join(
                            metadata[2:]), config.METADATA_FINAL))
                    new_text_file.write('\n')
                else:
                    text_lines = xl_file.parse(
                        sheet_name).to_string().split('\n')
                    first_line = True
        ...
        try:
            subprocess.call(['unoconv', '--format=txt',
                            upload_location + filename])
    ...
    for filename, label in zip(filenames, labels):
        try:
            has_not_error, text = import_annotated_text(
                request, lang, filename, label)
            if has_not_error:
                texts.append(text)
            else:
...
def annotate_uploaded_file(request, lang, *args, **kwargs):
    ...
    if use_paste:
        ...
        options = get_optparse(request, upload_location +
                               filename, tmp_dir, custom_filename=filename)
    ...
    try:
        for filename, label in zip(filenames, labels):
            ...
            try:
                text = import_text(request, lang, label,
                                   annotated_file_path, text_eligible)
                texts.append(text)
                if label:
...
def set_texts(texts):
    ...
    text_stats_list = json.loads(serializers.serialize(
        'json', TextStats.objects.filter(text_id__in=text_ids)))
...
def insert_label(path, label):
    ...
    with open(path, 'r', encoding='utf-8-sig') as f:
        ...
        while line:
            lines.append(line)
            line = f.readline()
    meta = config.METADATA_DELIMITER_LEBAL.join(
        [config.METADATA_DELIMITER_TAG.join([tag, value]) for tag, value in label.items()])
