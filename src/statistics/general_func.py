...
"""
The general features are included as follows:
1. token
2. word
3. word length
4. spelling error
5. sentences
6. sentence length (n words) (excluding punctuations)
7. paragraphs
8. paragraph length (n words) (excluding punctuations)
9. paragraph length (n sentences)
"""
...
from model.models import Paragraph
from scripts.helpers import eval_str
from scripts.statistics import mean, median, get_text_list
...
def _syllable_count_sv(word):
    """count the number of syllables in a Swedish word"""
    word = word.lower()
    vowels = 'aoueiyåöä'
...
def merge_defaultdicts_list(dicts):
    """
    Merge one or several default dictionaries 
    that have list as their values 
    into a single one
    """
...
def merge_defaultdicts(dicts):  # assumption: int, sum values with the same key
    """
    Merge one or serveral default dictionaries
    that have integer as their values
    into a single default dictionary
    """
    keys = [key for d in dicts for key in d.keys()]
...
def count_helper(blocks):  # blocks refer to sentences or paragraphs
    """
    A meta function to extract the desired feature 
    or cache a set of middle-layer data that can be used in a higher linguistic level
    based on a specific linguistic levels
    """
    ...
    polysyllables = sum([eval_str(block.general)['polysyllables']
                        for block in blocks])
    ...
    token_len_list = [0] * max(
        [len(eval_str(block.general)['token_len_list']) for block in blocks]
    )

    sentence_len_list = []
    freq_form_dict_upos = merge_defaultdicts(
        [eval_str(block.general)['freq_form_dict_upos'] for block in blocks])
    freq_norm_dict_upos = merge_defaultdicts(
        [eval_str(block.general)['freq_norm_dict_upos'] for block in blocks])
    freq_lemma_dict_upos = merge_defaultdicts(
        [eval_str(block.general)['freq_lemma_dict_upos'] for block in blocks])
    freq_form_dict_xpos = merge_defaultdicts(
        [eval_str(block.general)['freq_form_dict_xpos'] for block in blocks])
    freq_norm_dict_xpos = merge_defaultdicts(
        [eval_str(block.general)['freq_norm_dict_xpos'] for block in blocks])
    freq_lemma_dict_xpos = merge_defaultdicts(
        [eval_str(block.general)['freq_lemma_dict_xpos'] for block in blocks])
...
def get_args(content, level, _syllable_count_f=False):
    """
    Output the feature values based on 
    content, which is current input
    level, the given linguitic level (sentence, paragraph, text, texts)
    """
    if level == 'sent':
        ...
        if level == 'para':
            chars, tokens, words, syllables, polysyllables, types, \
                misspells, compounds, token_len_list, sentence_len_list, \
                freq_form_dict_upos, freq_norm_dict_upos, freq_lemma_dict_upos, \
                freq_form_dict_xpos, freq_norm_dict_xpos, freq_lemma_dict_xpos = count_helper(
                    content.sentences)
        else:
            chars, tokens, words, syllables, polysyllables, types, \
                misspells, compounds, token_len_list, sentence_len_list, \
...
def get_median(freq_list):
    """
    Generate a median given a frequency list
    """
...
class Count_features:
    ...
    def __init__(self, content, lang, level='sent'):
        ...
        self.chars, self.syllables, self.tokens, self.words, self.sents, \
            ...
            self.freq_form_dict_xpos, self.freq_norm_dict_xpos, self.freq_lemma_dict_xpos = get_args(
                content, level, _syllable_count_f)

        features = [
            # token_count, {'punct':punct}, 'scalar'),
            ('Token-count', self.tokens),
            # type_count, {'punct':punct}, 'raw'),
            ('Type-count', len(self.types)),
            # misspells_count) #, {}, 'scalar'),
            ('Spelling errors', self.misspells),
            # compound_count, {}, 'scalar'),
            ('Compound errors', self.compounds)
        ]  # .length_count_char, {'punct':punct}, 'raw')
        if self.token_len_list:
            features.append(('Word length', [round(
                self.chars/self.tokens, 2), round(get_median(self.token_len_list), 2)]))
        else:
            features.append(('Word length', [0, 0]))

        if level in ['para', 'text', 'texts']:
            # sentence_count, {}, 'scalar'))
            features.append(('Sentences', self.sents))
        ...
        if level.startswith('text'):
            ...
            if level == 'text':
                ...
                try:
                    paragraph_list = [
                        p for text in content for p in text.paragraphs]
                except Exception:
                    paragraph_list = [
                        p for text in content for p in Paragraph.objects.all().filter(text=text)]
            for para in paragraph_list:
                self.paragraph_len_list_word.append(
                    eval_str(para.general)['tokens'])
                self.paragraph_len_list_sent.append(
                    eval_str(para.general)['sents'])
        ...
        for feat, value in features:
            if isinstance(value, list):
                ...
                if feat.startswith('Sentence') or feat.startswith('Paragraph'):
                    continue
                if level == 'para':
                    scalar_list = [eval_str(sent.general)['feats'][feat]
                                   for sent in content.sentences]
                elif level == 'text':
                    scalar_list = [eval_str(para.general)['feats'][feat]
                                   for para in content.paragraphs]
                elif level == 'texts':
                    scalar_list = [eval_str(text.general)[
                        'feats'][feat] for text in content]
                if scalar_list:
                    self.average[feat] = {'mean': mean(
...
def get_pos(request, poses, text_id=False, **kwarg):
    ...
    if level == 'texts':
        context = merge_defaultdicts(
            [eval_str(text.general)[dict_name] for text in content])
    else:
    ...
    context = OrderedDict({key: value for key, value in sorted(
        context.items(), key=lambda x: x[1], reverse=True)})
    for pos in context:
...
def get_tokens(request, word_type, poses, text_id=False, **kwargs):
    ...
    if level == 'texts':
        if word_type.lower() == 'form':
            context = merge_defaultdicts(
                [eval_str(text.general)['freq_form_dict'] for text in content])
        elif word_type.lower() == 'norm':
            context = merge_defaultdicts(
                [eval_str(text.general)['freq_norm_dict'] for text in content])
        elif word_type.lower() == 'lemma':
            context = merge_defaultdicts(
                [eval_str(text.general)['freq_lemma_dict'] for text in content])
    else:
        if word_type.lower() == 'form':
