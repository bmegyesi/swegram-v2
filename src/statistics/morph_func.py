...
def function_tokens_incsc(sentence, lex=False, lex_to_non_lex=False, raw=True):
    ...
    for token in sentence.tokens:
        # a set of open class words
        if token.upos not in ["NOUN", "ADJ", "ADV", "VERB", "PROPN", "INTJ"]:
    ...
    functions = {
        'VERBFORM': [
            ...
            ('Present Participle to VERB',
             verbs_to_verbs, {'verb_type': 'Pres_part'}),
            ('Past Participle to VERB', verbs_to_verbs,
             {'verb_type': 'Past_part'}),
        ...
        'MultiPoS-MultiPoS': [
            ...
            ('Lex to Non-Lex', function_tokens_incsc,
             {'lex_to_non_lex': True}),
    ...
    def __init__(self, content, lang, algorithm='', level='sent'):
        ...
        if lang == 'en':
            ...
            self.functions['VERBFORM'] = [
                ...
                ('Past Participle to VERB', verbs_to_verbs,
                 {'verb_type': 'Past_part'}),
                ('Present VERB to VERB', verbs_to_verbs,
                 {'verb_type': 'VB|PRS'}),
                ('Past VERB to VERB', verbs_to_verbs, {'verb_type': 'VB|PRT'}),
                ('Supine VERB to VERB', verbs_to_verbs,
                 {'verb_type': 'VB|SUP'}),
                # ('S-VERB to VERB', s_verbs_incsc, {"verb":True}) #Swedish only
            ]
            ...
            self.functions['VERBFORM'] = [
                ...
                ('Supine VERB to VERB', verbs_to_verbs,
                 {'verb_type': 'VB|SUP'}),
                ('S-VERB to VERB', s_verbs_incsc,
                 {"verb": True})  # Swedish only
            ]
            self.functions['SubPoS_ALL'] = [
                ('S-VERB INCSC', s_verbs_incsc, {}),  # swedish only
                ('Neuter Gender NOUN INCSC',
                 neuter_gender_noun_incsc, {}),  # swedish only
                # ('S-VERB INCSC', thirdSG_pronoun_incsc, {})                 #english only
            ]

        self.categories = [
            ('VERBFORM', self.feats[0]['data']
             ), ('PoS-PoS', self.feats[1]['data']),
            ('SubPoS-ALL', self.feats[2]['data']
             ), ('PoS-ALL', self.feats[3]['data']),
                ...
                for name, func, kwarg in self.functions[category_name]:
                    if level == 'sent':
                        ...
                        for sent in content.sentences:
                            sc, st = eval_str(sent.morph)[
                                'feats'][self.dict2index[category_name]]['data']['$'.join([name, 'raw'])]
                            c += sc
                            t += st
                            sent_scalars.append(eval_str(sent.morph)[
                                                'feats'][self.dict2index[category_name]]['data'][name])
                        self.sent_raw[name] = sent_scalars

                    elif level == 'texts':
                        c, t = 0, 0
                        for text in content:
                            sc, st = eval_str(text.morph)[
                                'feats'][self.dict2index[category_name]]['data']['$'.join([name, 'raw'])]
                            c += sc
                            t += st
                ...
                for name, _, _ in self.functions[category_name]:
                    ...
                    if level != 'texts':
                        for sentence in content.sentences:
                            values.append(eval_str(sentence.morph)[
                                          'feats'][self.dict2index[category_name]]['data'][name])
                        feature_list[name] = {'mean': mean(
                            values), 'median': median(values)}
                    else:
                        for text in content:
                            values.extend(eval_str(text.morph)
                                          ['sent_raw'][name])
                        feature_list[name] = {'mean': mean(
                            values), 'median': median(values)}
