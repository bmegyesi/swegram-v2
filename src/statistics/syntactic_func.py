...
def is_a_ud_tree(sentence, upload_annotated=False):
    if upload_annotated:
        # This is used to check the sentence that has been annotated before
        heads = [int(token.head) for token in sentence]
    else:
    ...
    if len(children) > len(set(children)):
        ...
        print("Same indeces for two nodes in sentence %s." %
              (sentence.text_id))
        return False
...
def dep_length_per_sentence(sentence):
    ...
    dep_length_nos = [len(path)-1 for path in dep_lengths]
...
# find all children that have i as their direct or indirect head.
def find_children(heads, i):
    children = []
    while i in heads:
        child = heads.index(i)
...
class Syntactic_features:
    ...
    def __init__(self, content, algorithm='', level='sent'):
        functions = [
            ...
            ('Ratio of right dependency arcs', dep_dir_count,
             {'dir': 'right', 'ratio': False}, 'two', 'cent'),
            ('Ratio of left dependency arcs', dep_dir_count,
             {'dir': 'left', 'ratio': False}, 'two', 'cent'),
            ('Modifier variation', mod_incsc, {}, 'two', 'incsc'),  # INCSC
            ('Pre-modifier INCSC', mod_incsc,
             {'pre_mod': True}, 'two', 'incsc'),  # INCSC
            ('Post-modifier INCSC', mod_incsc,
             {'post_mod': True}, 'two', 'incsc'),  # INCSC
        ...
        if level == 'sent':
            ...
            content = [text for text in content if eval_str(text.syntactic)[
                'empty'] == False]
            if not content:
                self.empty = True
                    ...
                    if args == 'one':
                        if level == 'sent':
                            if name == 'Dependency length':
                                mean_scalar, median_scalar = func(
                                    content, average=True)
                                self.feats['$'.join([name, 'average'])] = {
                                    'mean': mean_scalar, 'median': median_scalar}
                            ...
                            for sent in content:
                                sent_scalars.append(
                                    eval_str(sent.syntactic)['feats'][name])
                            self.feats[name] = sum(
                                sent_scalars) if incsc == 'sum' else max(sent_scalars)
                            ...
                            for text in content:
                                text_scalars.append(
                                    eval_str(text.syntactic)['feats'][name])
                            self.feats[name] = sum(
                                text_scalars) if incsc == 'sum' else max(text_scalars)
                        ...
                        if level == 'sent':
                            ...
                            for sent in content:
                                sc, st = eval_str(sent.syntactic)[
                                    'sent_raw']['$'.join([name, 'raw'])]
                                c += sc
                                t += st
                                sent_scalars.append(
                                    eval_str(sent.syntactic)['feats'][name])
                        ...
                        if incsc == 'cent':
                            ...
                            # set 1000 as the ceiling
                            scalar = round(c/t*1000, 2) if t else 1000
                ...
                for name, _, _, args, _ in functions:
                    ...
                    if level != 'texts':
                        for sentence in content:
                            values.append(
                                eval_str(sentence.syntactic)['feats'][name])
                        self.feats[name] = {'mean': mean(
                            values), 'median': median(values)}
                    else:
                        for text in content:
                            values.extend(eval_str(text.syntactic)[
                                          'sent_raw'][name])
                        self.feats[name] = {'mean': mean(
                            values), 'median': median(values)}
