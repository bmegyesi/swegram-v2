...
const useSelect = (props, emit) => {
  ...
  const isFilterMethodValid = vue.computed(
    () => props.filterable && shared.isFunction(props.filterMethod)
  );
  const isRemoteMethodValid = vue.computed(
    () => props.filterable && props.remote && shared.isFunction(props.remoteMethod)
  );
  ...
  const dropdownMenuVisible = vue.computed({
    get() {
      return expanded.value && (props.loading || !isRemoteSearchEmpty.value) && (!debouncing.value || !types.isEmpty(states.previousQuery));
    },
    set(val) {
      expanded.value = val;
    }
  ...
  const update = (val) => {
    emit(event$1.UPDATE_MODEL_EVENT, val);
    emitChange(val);
    states.previousValue = props.multiple ? String(val) : val;
    vue.nextTick(() => {
      if (props.multiple && shared.isArray(props.modelValue)) {
        const cachedOptions = states.cachedOptions.slice();
        const selectedOptions = props.modelValue.map(
          (value) => getOption(value, cachedOptions)
        );
  ...
  const handleDel = (e) => {
    ...
    if (!props.multiple)
      return;
    if (code === aria.EVENT_CODE.delete)
      return;
    if (states.inputValue.length === 0) {
      e.preventDefault();
  ...
  const getOption = (value, cachedOptions) => {
    ...
    if (cachedOptions && cachedOptions.length) {
      const option = cachedOptions.find(
        (option2) => getValueKey(getValue(option2)) === selectValue
      );
      if (option) {
        return option;
      }
    ...
    if (props.multiple) {
      ...
      if (hasModelValue.value) {
        ...
        const selectedItemIndex = options.findIndex(
          (option) => getValueKey(getValue(option)) === getValueKey(props.modelValue)
        );
