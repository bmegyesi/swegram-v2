...
function useEvent(props, emit) {
  ...
  const handleMouseMove = (event, column) => {
    ...
    const target = types.isElement(el) ? el.closest("th") : null;
    if (!target) {
      return;
    }
    const isSortable = style.hasClass(target, "is-sortable");
    if (isSortable) {
  ...
  const handleMouseOut = () => {
    if (!core.isClient || dragging.value)
      return;
    document.body.style.cursor = "";
  };
  ...
  const handleSortClick = (event, column, givenOrder) => {
    ...
    if (["ascending", "descending"].some(
      (str) => style.hasClass(clickTarget, str) && !column.sortOrders.includes(str)
    )) {
      return;
    }
