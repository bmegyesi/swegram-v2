...
var useGridTouch = require('../hooks/use-grid-touch.js');
...
const createGrid = ({
  ...
  return vue.defineComponent({
    ...
    setup(props, { emit, expose, slots }) {
      ...
      const { onWheel } = useGridWheel.useGridWheel(
        {
          ...
          atXEndEdge: vue.computed(
            () => states.value.scrollLeft >= estimatedTotalWidth.value - vue.unref(parsedWidth)
          ),
          atYStartEdge: vue.computed(() => states.value.scrollTop <= 0),
          atYEndEdge: vue.computed(
            () => states.value.scrollTop >= estimatedTotalHeight.value - vue.unref(parsedHeight)
          )
        },
        (x, y) => {
          var _a, _b, _c, _d;
      ...
      const { touchStartX, touchStartY, handleTouchStart, handleTouchMove } = useGridTouch.useGridTouch(
        windowRef,
        states,
        scrollTo,
        estimatedTotalWidth,
      ...
      const renderInner = () => {
        ...
        return [
          vue.h(
            Inner,
            vue.mergeProps(props.innerProps, {
      ...
      const renderWindow = () => {
        const Container = vue.resolveDynamicComponent(
