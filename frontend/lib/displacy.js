...
class displaCy {
  ...
  render(parse, settings = {}, text) {
    ...
    // Partially encode the result SVG str for better readability
    const result = `data:image/svg+xml,${newElement.outerHTML.replace(/"/g, "'").replace(/%/g, '%25').replace(/#/g, '%23').replace(/{/g, '%7B')
      .replace(/}/g, '%7D')
      .replace(/</g, '%3C')
      .replace(/>/g, '%3E')}`;
    return result;
  ...
  _el(tag, options) {
    const {
      classnames = [], attributes = [], style = [], children = [], text, id, xlink,
    } = options;
